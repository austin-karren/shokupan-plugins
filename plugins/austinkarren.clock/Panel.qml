...
Panel {
  ...
  // SHOKUPAN: date clicks open GNOME Calendar on that day (ADR-0006) — the
  // popup engine is calendar-toggle, by ABSOLUTE path: bar.run goes through
  // bash -lc, which does not put ~/.local/bin on PATH.
  function openCalendarApp(dateKey) {
    root.close()
    if (root.bar) root.bar.run(Quickshell.env("HOME") + "/.local/bin/calendar-toggle --date " + dateKey)
  }
        ...
        Column {
          ...
          Item {
            ...
            MouseArea {
              ...
              width: heroRow.width
              height: heroRow.height
              // SHOKUPAN: always enabled — browsing keeps upstream's
              // back-to-today; on today's month the hero opens the calendar.
              enabled: true
              hoverEnabled: true
              cursorShape: Qt.PointingHandCursor
              onClicked: root.viewingCurrentMonth
                ? root.openCalendarApp(root.todayKey)
                : root.goToToday()

              PanelToolTip {
                visible: heroMouse.containsMouse
                text: root.viewingCurrentMonth ? "Open GNOME Calendar" : "Back to today"
                ...
                Row {
                  ...
                  Repeater {
                    ...
                    Rectangle {
                      ...
                      // SHOKUPAN: hover fill — the grid is a picker here.
                      ...
                      // SHOKUPAN: day cells open GNOME Calendar on that date.
                      MouseArea {
                        id: dayMouse
                        anchors.fill: parent
                        hoverEnabled: true
                        cursorShape: Qt.PointingHandCursor
                        onClicked: root.openCalendarApp(modelData.key)
                      }
