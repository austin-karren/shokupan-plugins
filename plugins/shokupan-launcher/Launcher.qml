// SHOKUPAN FORK of /usr/share/omarchy/shell/plugins/launcher/Launcher.qml
// (ADR-0027). This file is byte-identical to upstream's — the one functional
// change lives in the sibling LauncherSearch.js (commands-first partition).
// The namespace omarchy.* is reserved (PluginRegistry rejects shadowing), so
// the merged list has to be its own overlay plugin; upstream's launcher cannot
// be patched in place and /usr/share is not ours to edit. After an omarchy
// update, re-diff both files against upstream and re-apply the partition.
import Quickshell
